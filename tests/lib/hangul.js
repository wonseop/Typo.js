...
(function() {
	...
	function _isConsonant( charactor ) {
		return CONSONANTS_HASH[ charactor ];
	}

	function _isCho( charactor ) {
		return CHO_HASH[ charactor ] !== undefined;
	}

	function _isJung( charactor ) {
		return JUNG_HASH[ charactor ] !== undefined;
	}

	function _isJong( charactor ) {
		return JONG_HASH[ charactor ] !== undefined;
	}

	function _isHangul( codeNumber ) {
		return 0xAC00 <= codeNumber && codeNumber <= 0xd7a3;
	}
 ...
 var rCho =
    ...
    for ( var i = 0; i < string.length; i++ ) {
      var nTmp = string.charCodeAt(i) - 0xAC00;
      jong = nTmp % 28; // 종성
